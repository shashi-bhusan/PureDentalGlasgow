(function($) {
	...
	/* ==========================================================================
	   Initialize Photo viewer for product post
	   ========================================================================== */
	   if( $('#photo-viewer').length > 0 ) {
		   ...
		   $('.thumb').on('click', function(e) {
			  ...
			  $('#pm-photo-viewer-loader').css({'opacity' : 1}); //Fade in loading icon
			
			  if (cache.hasOwnProperty(src)) {        // If cache contains this image
				if (cache[src].isLoading === false) { // And if isLoading is false
				  methods.crossfade(cache[src].$img); // Call crossfade() function
				}
				...
				$img.on('load', function(){           // When image has loaded
				  ...
				  if (request === src) {
					methods.crossfade($img);          // Call crossfade() function
		   ...
		   //Simulate click on first thumb to load first image
		   $('.thumb').eq(0).click();
